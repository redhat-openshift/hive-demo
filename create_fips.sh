...
if [[ -z $CLUSTER_NAME || -z $AWS_DOMAIN || -z $OSP_NETWORK ]] ; then
  ...
  # export CLUSTER_NAME=${CLUSTER_NAME:-"my-cluster"} # To set the cluster Subdomain (A Record) in AWS.
  # export AWS_DOMAIN=${AWS_DOMAIN:-"my.domain.com"} # To set the cluster Domain in AWS.
  # export OSP_NETWORK=${OSP_NETWORK:-"osp_net_1"} # The external network for the new Floating IPs on OSP.
  "
  exit 1
else
